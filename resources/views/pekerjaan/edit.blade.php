    ...
    <div class="py-10 bg-gray-400 h-auto">
        <div class="max-w-7xl mx-auto h-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl mt-6 mx-6 overflow-hidden shadow-xl sm:rounded-lg">
                <form action="{{ route('pekerjaan.update', $pekerjaans->id) }}" method="POST" class="my-8 mx-8">
                    @csrf
                    @method('PUT')
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" name="name_pekerjaan" id="name_pekerjaan"
                            value="{{ $pekerjaans->name_pekerjaan }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2
